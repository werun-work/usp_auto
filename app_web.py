...
def get_data_bulldozer(target_url, max_pages=10):
    ...
    # 🔥 4개 변수 반환 (ValueError 원천 차단)
    return brand_text, "\n".join(review_list)[:30000], pot_imgs, p_name 

# ==========================================
# [4. AI 분석 엔진] 🔥 모호성 금지 & / 줄바꿈 규칙 추가
# ==========================================
def analyze_deep_usp_summarized(brand_text, review_text, pot_imgs, content_type, copy_style, product_url, product_name, user_ref_copy):
    ...
    copy_quality_rule = """
    [카피 작성 절대 규칙]
    1. '이 가격?', '세트 가격', '득템 기회', '역대급' 등 추상적이고 모호한 표현은 절대 금지합니다.
    2. '2장에 29,900원', '입는 순간 -5kg 슬림핏'처럼 구체적인 숫자, 가격, 명확한 혜택을 명시하세요.
    3. 모든 줄바꿈이 필요한 곳에는 <br> 대신 / 기호를 사용하세요.
    """
    ...
    *(AI 내부 지시사항: {ai_instruction})*
    ...
    return "🚨 [전체 서버 폭주] 10~20초 뒤 다시 시도해 주세요.", None

# ==========================================
# [추가/비교 카피 무한 생성기] 
# ==========================================
def generate_extra_copies(base_report, user_req, copy_style, user_ref_copy):
    rule = "공백 포함 25자 이내, 구체적인 가격/수치 정보 포함(추상적 표현 금지), 줄바꿈은 / 사용"
    if "명사/동사" in copy_style: rule = "공백 포함 20자 이내, 명사/동사 종결, 추상적 표현 금지, 줄바꿈 / 사용"
    ...
    return "🚨 서버 폭주로 추출에 실패했습니다."

def generate_compare_copy(base_report, cmp_style):
    rule = "공백 포함 25자 이내, 구체적인 가격/수치 포함(추상적 표현 금지), 줄바꿈은 / 사용"
    if "명사/동사" in cmp_style: rule = "공백 포함 20자 이내, 명사/동사 종결, 추상적 표현 금지, 줄바꿈 / 사용"
...
# ==========================================
if check_password():
    st.title("🎯 마케팅 USP & 카피 자동 추출기 (V15.8 Finale)")
    st.markdown("---")
        ...
        if st.session_state.analyzed:
            ...
            if "이미지" in st.session_state.content_type:
                ...
                with col_ad1:
                    ...
                    st.info("💡 **[Ctrl+V 붙여넣기 확실한 방법]**\n1. 이미지를 캡처(Ctrl+C)합니다.\n2. 파일 업로드 박스가 아닌, **화면의 빈 하얀색 바탕 아무 곳이나 마우스로 한 번 클릭**합니다.\n3. 그 상태에서 `Ctrl+V`를 누르면 이미지가 쏙 들어갑니다! (안 되면 아래 팝업창 열어서 파일 첨부)")
    ...
    st.markdown("<br><center>Internal Marketing Tool V15.8 (The Final Fixes)</center>", unsafe_allow_html=True)
