...
def analyze_deep_usp_summarized(brand_text, review_text, pot_imgs, content_type, copy_style, product_url, product_name, user_ref_copy):
    ...
    if "명사/동사" in copy_style:
        ...
        ui_display_text = "*카피 텍스트 기준 '공백 포함 25자 이내 추출'*"
        ai_instruction = f"{copy_quality_rule}\n대괄호 [유형] 부분을 제외한 실제 카피 텍스트는 공백 포함 25자 이내로 하되, 고객에게 말 거는 듯한 자연스러운 서술형으로 작성할 것."
    
    ref_section = "[자사 베스트 카피 레퍼런스]\n- 입는 순간 -5kg, 마법의 슬림핏\n- 물놀이, 운동, 외출 올인원!\n- 남편이랑 아들이 서로 입겠다고 싸워요"
    ...
    # 지침:
    1. 인사말 없이 바로 본론 시작.
    2. 첫 줄 제목 포맷: [{product_name} 핵심 USP & 후킹 카피 제안 / {date_str}]
    3. 1번(소구점 요약)과 2번(리뷰 요약) 항목은 최소 5개 이상 풍성하게 도출.
    {ref_section}

    ---
    # Input: 상세페이지 {brand_text} / 리뷰 {review_text if len(review_text) > 50 else "리뷰 없음"}
    ---
    ...
    * **[대표 리뷰]**: (생생한 한 마디)
    ...
    [작성 규칙]
    - 3번 항목 타이틀 바로 아래에는 오직 "{ui_display_text}" 문구만 적으세요.
    - {ai_instruction}
...
# ==========================================
# [추가/비교 카피 엔진] 🔥 재시도 5회 및 구체성 강화
# ==========================================
def generate_extra_copies(base_report, user_req, copy_style, user_ref_copy):
    rule = "공백 포함 25자 이내, 구체적 정보 포함, 모호한 가격 질문 금지, 줄바꿈은 / 기호 사용"
    prompt = f"당신은 카피라이터입니다. 서론 없이 깔끔하게 새 카피 8줄만 출력하세요.\n기존분석: {base_report[:1500]}\n요청: {user_req}\n규칙: {rule}"
    client = genai.Client(api_key=MY_GEMINI_API_KEY)
    for attempt in range(5):
        try: return client.models.generate_content(model='gemini-3.1-flash', contents=prompt).text
        except: time.sleep(1); continue
    return "🚨 추출 실패"

def generate_compare_copy(base_report, cmp_style):
    rule = "공백 포함 25자 이내, 대괄호 뒤 '카피' 단어 사용 금지, 구체적 정보 명시, 줄바꿈은 / 기호 사용"
...
def create_ad_image(img_file, main_copy, sub_copy):
    ...
    try:
        img = Image.open(io.BytesIO(img_file.getvalue())).convert("RGBA")
        base_w = 1080
    ...
    with tab1:
        ...
        if st.button("▶ 분석 시작", type="primary", use_container_width=True):
            if not worker_input or not main_url_input:
                ...
                with st.spinner("🚀 AI가 데이터를 수집하고 분석하는 중입니다..."):
        ...
        if st.session_state.analyzed:
            ...
            if split_keyword in st.session_state.main_report_text:
                ...
                with col_left:
                    ...
                    for idx, ex in enumerate(st.session_state.extra_copies):
                        with st.expander(f"추가 추출 #{idx+1}"): st.markdown(ex['res'])
                with col_line: st.markdown("<div class='vertical-line'></div>", unsafe_allow_html=True)
                with col_right:
                    st.markdown("### 🔄 카피 스타일 비교 추출")
                    cmp_style = st.selectbox("스타일 선택", ["USP + 세일즈 후킹형", "명사/동사 임팩트형", "자연스러운 서술형"], index=0)
                    if st.button("✨ 비교 추출하기", use_container_width=True):
                        with st.spinner("비교 추출 중..."):
                    ...
                    for idx, cmp in enumerate(reversed(st.session_state.compare_copy_list)):
                        with st.expander(f"카피 비교 ({cmp['style']}) - #{len(st.session_state.compare_copy_list)-idx}"): st.markdown(cmp['res'])
            else:
            ...
            if "이미지" in st.session_state.content_type:
                ...
                with st.form("ad_plan_form"):
                    ...
                    if st.form_submit_button("💾 표 내용 저장"): st.success("저장 완료!")
                ...
                with col_ad2:
                    if st.session_state.ad_img:
                        ...
                        st.download_button("💾 시안 다운로드", data=st.session_state.ad_img, file_name="XEXY_AD.png")
            ...
            if st.button("🚀 모든 내용 하나로 합치기", use_container_width=True):
                ...
                if st.session_state.compare_copy_list:
                    final += "[비교 카피 내역]\n"
                    for cmp in reversed(st.session_state.compare_copy_list): final += f"▶ {cmp['style']}\n{cmp['res']}\n\n"
                if st.session_state.extra_copies:
                    final += "[추가 카피 내역]\n"
                    for ex in st.session_state.extra_copies: final += f"▶ {ex['req']}\n{ex['res']}\n\n"
                ...
                save_to_google_sheet([datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M'), f"[최종] {st.session_state.product_name}", st.session_state.p_code, st.session_state.main_url, final], st.session_state.worker_name)
                st.success("✅ 시트에도 최종본이 적재되었습니다!")
